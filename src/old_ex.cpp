...
int main(int argc, char **argv) {
	...
	//add details to code container from args
	code.boot = args->boot;
	code.r = args->d; // gets used to construct the code
	code.s = 4;
	code.l = args->d;
	code.t_check = args->t_check;
	code.big_t_max = args->big_t_max;
	code.probability = args->p;
	code.seed0 = args->s0;
	code.seed1 = args->s1;
	code.t_delete = args->t_delete;
	code.max_num_X = args->max_num_X;
	code.max_num_Z = args->max_num_Z;
	code.dir = args->ems;

	//generate the code information
	generate_surface_code();

	// Create the recipe for an infinite simulation
	recipe = qc_create_recipe_adv(RECIPE_INFINITE, code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(),1, FALSE);
...
SC_DP_QC *create_sc_dp_qc(ARGS *args, RECIPE_ADV *recipe) {
	...
	sc_dp_qc->d = d = args->d;
	sc_dp_qc->n = n = 2*d - 1;
	assert(n*n == code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size());
	...
	sc_dp_qc->frame = (int *)my_calloc(code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(), sizeof(int));
	// Create an n by n array of qubits
	//sc_dp_qc->q_arr = q_arr = (QUBIT ***)my_2d_calloc(n, n, sizeof(QUBIT *));
	sc_dp_qc->q_arr = q_arr = (QUBIT **)my_calloc(code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(), sizeof(QUBIT*));
	for (i = 0; i < code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(); i++) {
			q_arr[i] = qc_create_and_insert_qubit(sc_dp_qc->dp_qc->qc, i, 0, 0, QUBIT_HT_SIZE);
	}
	...
	sc_dp_qc->primal_boundary = (SET**) my_calloc(code.X_boundary.size(), sizeof(SET *));
	sc_dp_qc->dual_boundary = (SET**) my_calloc(code.Z_boundary.size(), sizeof(SET *));

	//TODO: currently only const elements in boundary container -> change to arbitrary
	sc_dp_qc->boundaries = (BALL **)my_calloc(8 , sizeof(BALL *));
	//primal
	for(int i = 0; i < code.X_boundary.size(); ++i){
		sc_dp_qc->primal_boundary[i] = create_boundary_set(qc, sc_dp_qc->boundaries, i, PRIMAL_BOUNDARY, -1-i, -1, 0);
	}
	//dual
	for(int i = 0; i < code.Z_boundary.size(); ++i){
		sc_dp_qc->dual_boundary[i] = create_boundary_set(qc, sc_dp_qc->boundaries, i + code.X_boundary.size(), DUAL_BOUNDARY, -1-i, -2, 0);
	}
	//temporal
	sc_dp_qc->temporal_primal_boundary = (SET**) my_calloc(2,sizeof(SET *));
	sc_dp_qc->temporal_dual_boundary = (SET**) my_calloc(2,sizeof(SET *));

	sc_dp_qc->temporal_primal_boundary[0] = create_boundary_set(qc, sc_dp_qc->boundaries, code.Z_boundary.size() + code.X_boundary.size(), PRIMAL_BOUNDARY, -1, -3, 0);
	sc_dp_qc->temporal_primal_boundary[1] = create_boundary_set(qc, sc_dp_qc->boundaries, code.Z_boundary.size() + code.X_boundary.size() + 1, PRIMAL_BOUNDARY, -2, -3, 0);

	sc_dp_qc->temporal_dual_boundary[0] = create_boundary_set(qc, sc_dp_qc->boundaries, code.Z_boundary.size() + code.X_boundary.size()+2, DUAL_BOUNDARY, -1, -4, 0);
	sc_dp_qc->temporal_dual_boundary[1] = create_boundary_set(qc, sc_dp_qc->boundaries, code.Z_boundary.size() + code.X_boundary.size()+3, DUAL_BOUNDARY, -2, -4, 0);


	qc->get_boundary = get_boundary;
	qc->boundaries = (void *)sc_dp_qc->boundaries;
	create_initial_syndrome_and_set_arrays(sc_dp_qc);
 ...
 */
void create_initial_syndrome_and_set_arrays(SC_DP_QC *sc_dp_qc) {
	int seti, setj;
	int i;
	SYNDROME **syn_arr;
	SET ***set_arr;

	syn_arr = (SYNDROME **)my_calloc(code.X_stabilizer.size(), sizeof(SYNDROME *));
	set_arr = (SET ***)my_2d_calloc(code.X_stabilizer.size(), 2, sizeof(SET *));

	for (i = 0; i < code.X_stabilizer.size(); i++) {
		seti = code.num_qubits + i;
		setj = 0;
		// Create and insert a new syndrome into the syndrome array
		syn_arr[i] = qc_create_syndrome();
		qc_insert_syndrome(sc_dp_qc->dp_qc->qc, syn_arr[i]);

		// Create a set with no boundary
		set_arr[i][1] = qc_create_set_adv(sc_dp_qc->dp_qc->qc, PRIMAL, seti, setj, 1, infer_boundary_helper(sc_dp_qc, i, false));
		set_arr[i][0] = qc_create_set_adv(sc_dp_qc->dp_qc->qc, PRIMAL, seti, setj, 2, infer_boundary_helper(sc_dp_qc, i, false));

		// Insert the new set into the set array, and associate the set with a syndrome
		qc_insert_set(sc_dp_qc->dp_qc->qc, set_arr[i][1]);
		qc_associate_syndrome(set_arr[i][1], syn_arr[i]);
		qc_insert_set(sc_dp_qc->dp_qc->qc, set_arr[i][0]);
	}
	...
	//DUAL: Z-Stabilizers
	syn_arr = (SYNDROME **)my_calloc(code.Z_stabilizer.size(), sizeof(SYNDROME *));
	set_arr = (SET ***)my_2d_calloc(code.Z_stabilizer.size(), 2, sizeof(SET *));

	for (i = 0; i < code.Z_stabilizer.size(); i++) {
		seti = code.num_qubits + code.X_stabilizer.size() + i;
		setj = 0;
		// Create and insert a new syndrome into the syndrome array
		syn_arr[i] = qc_create_syndrome();
		qc_insert_syndrome(sc_dp_qc->dp_qc->qc, syn_arr[i]);
		...
		// Create a set with no boundary
		set_arr[i][1] = qc_create_set_adv(sc_dp_qc->dp_qc->qc, DUAL, seti, setj, 1, infer_boundary_helper(sc_dp_qc, i, true));
		set_arr[i][0] = qc_create_set_adv(sc_dp_qc->dp_qc->qc, DUAL, seti, setj, 2, infer_boundary_helper(sc_dp_qc, i, true));
...
SC_DP_QC *copy_sc_dp_qc(SC_DP_QC *sc_dp_qc) {
	...
	*sc_dp_qc2 = *sc_dp_qc;
	...
	// Copy the frame
	sc_dp_qc2->frame = (int *)my_calloc(code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(), sizeof(int));
	for (i = 0; i < code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(); i++) {
		sc_dp_qc2->frame[i] = sc_dp_qc->frame[i];
	}

	// Copy the qubits
	sc_dp_qc2->q_arr = (QUBIT **)my_calloc(code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(), sizeof(QUBIT *));
	for (i = 0; i < code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(); i++) {
		sc_dp_qc2->q_arr[i] = sc_dp_qc->q_arr[i]->copy;
	}

	// Copy the PRIMAL syndrome and set arrays 
	sc_dp_qc2->syn_arr_pr = (SYNDROME **)my_calloc(code.X_stabilizer.size(), sizeof(SYNDROME *));
	sc_dp_qc2->set_arr_pr = (SET ***)my_2d_calloc(code.X_stabilizer.size(), 2, sizeof(SET *));
	for (i = 0; i < code.X_stabilizer.size(); i++) {
		sc_dp_qc2->syn_arr_pr[i] = sc_dp_qc->syn_arr_pr[i]->copy;
		sc_dp_qc2->set_arr_pr[i][0] = sc_dp_qc->set_arr_pr[i][0]->copy;
		sc_dp_qc2->set_arr_pr[i][1] = sc_dp_qc->set_arr_pr[i][1]->copy;
	}
	
	// Copy the DUAL syndrom and set arrays
	sc_dp_qc2->syn_arr_du = (SYNDROME **)my_calloc(code.Z_stabilizer.size(), sizeof(SYNDROME *));
	sc_dp_qc2->set_arr_du = (SET ***)my_2d_calloc(code.Z_stabilizer.size(), 2, sizeof(SET *));
	for (i = 0; i < code.Z_stabilizer.size(); i++) {
		sc_dp_qc2->syn_arr_du[i] = sc_dp_qc->syn_arr_du[i]->copy;
		sc_dp_qc2->set_arr_du[i][0] = sc_dp_qc->set_arr_du[i][0]->copy;
		sc_dp_qc2->set_arr_du[i][1] = sc_dp_qc->set_arr_du[i][1]->copy;
	}
...
void free_sc_dp_qc(SC_DP_QC *sc_dp_qc) {
	...
	free((void *)sc_dp_qc->frame);

	// Free the array of qubits (the qubits themselves are owned by the qc)
	free((void *)sc_dp_qc->q_arr);
	
	// Free the primal syndrome array and the syndromes
	for (i = 0; i < code.X_stabilizer.size(); i++) {
		qc_free_syndrome(sc_dp_qc->syn_arr_pr[i]);
	}
	free((void *)sc_dp_qc->syn_arr_pr);
	
	// Free the dual syndrome array and the syndromes
	for (i = 0; i < code.Z_stabilizer.size(); i++) {
			qc_free_syndrome(sc_dp_qc->syn_arr_du[i]);
	}
	free((void *)sc_dp_qc->syn_arr_du);

	// Free the primal and dual set arrays
	my_2d_free(code.X_stabilizer.size(), (void **)sc_dp_qc->set_arr_pr);
	my_2d_free(code.Z_stabilizer.size(), (void **)sc_dp_qc->set_arr_du);

	// Free the sets associated with the boundaries
	for(i=0; i < code.X_boundary.size(); ++i){
		qc_free_bdy(sc_dp_qc->primal_boundary[i]);
	}
	for(i=0; i < code.Z_boundary.size(); ++i){
		qc_free_bdy(sc_dp_qc->dual_boundary[i]);
	}

	qc_free_bdy(sc_dp_qc->temporal_primal_boundary[0]);
	qc_free_bdy(sc_dp_qc->temporal_primal_boundary[1]);

	qc_free_bdy(sc_dp_qc->temporal_dual_boundary[0]);
	qc_free_bdy(sc_dp_qc->temporal_dual_boundary[1]);
...
void measure_stabilizers(SC_DP_QC *sc_dp_qc, long int big_t) {
	...
	init_X_syn_dead_Z_syn(dp_qc, q_arr);

	// Change of basis (Hadamard) on X-syndromes, Initialize Z-syndromes in Z-basis
	H_X_syn_init_Z_syn(dp_qc, q_arr);

	prepare_measurementX(dp_qc, q_arr);
	prepare_measurementZ(dp_qc, q_arr);

	H_X_syn_meas_Z_syn(sc_dp_qc, big_t, dp_qc, q_arr);
	meas_X_syn_dead_Z_syn(sc_dp_qc, big_t, dp_qc, q_arr);
...
void init_X_syn_dead_Z_syn(DP_QC *dp_qc, QUBIT **q_arr) {
   	int i;
	for (i=0; i < code.num_qubits; ++i){
		dp_iden_init_Z(dp_qc, q_arr[i]);
	}
	for (i = code.num_qubits; i < code.num_qubits + code.X_stabilizer.size(); ++i){
		dp_init_Z(dp_qc, q_arr[i]);
	}
	for (i = code.num_qubits + code.X_stabilizer.size(); i < code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(); ++i) {
		dp_dead_H(dp_qc, q_arr[i]);
	}
}

void H_X_syn_init_Z_syn(DP_QC *dp_qc, QUBIT **q_arr) {
	int i;
	for (i=0; i < code.num_qubits; ++i){
		dp_iden_init_Z(dp_qc, q_arr[i]);
	}
...
void custom_cnot(int control, int target, DP_QC *dp_qc, QUBIT **q_arr){
	dp_cnot(dp_qc, q_arr[control], q_arr[target]);
	for(int i = 0; i < code.num_qubits + code.X_stabilizer.size() + code.Z_stabilizer.size(); ++i){
		if(i != control && i != target){
			dp_dead_cnot(dp_qc,q_arr[i]);
		}
	}
}

void prepare_measurementX(DP_QC *dp_qc, QUBIT **q_arr){
	int i;
	for(i=0; i <code.X_stabilizer.size(); ++i){
		for(auto qubit : code.X_stabilizer.at(i)){
			custom_cnot(code.num_qubits + i, qubit, dp_qc, q_arr);
		}
	}
}
void prepare_measurementZ(DP_QC *dp_qc, QUBIT **q_arr){
	int i;
	for(i=0; i< code.Z_stabilizer.size(); ++i){
		for(auto qubit : code.X_stabilizer.at(i)){
			custom_cnot(qubit, i + code.num_qubits + code.X_stabilizer.size(), dp_qc, q_arr);
		}
	}
}
 ...
 */
void H_X_syn_meas_Z_syn(SC_DP_QC *sc_dp_qc, long int big_t, DP_QC *dp_qc, QUBIT **q_arr) {
	int i, seti, setj;
	SYNDROME **syn_arr_du = sc_dp_qc->syn_arr_du;
	SET ***set_arr_du = sc_dp_qc->set_arr_du;
	...
	// Z stabilizers
	for (i = 0; i < code.Z_stabilizer.size(); ++i) {
		seti = i + code.num_qubits + code.X_stabilizer.size();
		setj = 0;
		dp_meas_Z(dp_qc, q_arr[seti], set_arr_du[i][lay1], set_arr_du[i][lay2]);
		qc_unassociate_syndrome(syn_arr_du[i]);
		qc_associate_syndrome(set_arr_du[i][lay2], syn_arr_du[i]);
		set_arr_du[i][lay1] = qc_create_set_adv(sc_dp_qc->dp_qc->qc, DUAL, seti, setj, 2, infer_boundary_helper(sc_dp_qc, i, true));
		qc_insert_set(dp_qc->qc, set_arr_du[i][lay1]);
	}
}
 ...
 * \param[in] q_arr The array of \ref qubit%s to perform the gates on 
 */
void meas_X_syn_dead_Z_syn(SC_DP_QC *sc_dp_qc, long int big_t, DP_QC *dp_qc, QUBIT **q_arr) {
	int i, seti, setj;
	SYNDROME **syn_arr_pr = sc_dp_qc->syn_arr_pr;
	SET ***set_arr_pr = sc_dp_qc->set_arr_pr;
	...
	for (i = 0; i < code.X_stabilizer.size(); ++i) {
		seti = i + code.num_qubits;
		setj = 0;
		dp_meas_Z(dp_qc, q_arr[seti], set_arr_pr[i][lay1], set_arr_pr[i][lay2]);
		qc_unassociate_syndrome(syn_arr_pr[i]);
		qc_associate_syndrome(set_arr_pr[i][lay2], syn_arr_pr[i]);
		set_arr_pr[i][lay1] = qc_create_set_adv(dp_qc->qc, PRIMAL, seti, setj, 2, infer_boundary_helper(sc_dp_qc, i, false));
		qc_insert_set(dp_qc->qc, set_arr_pr[i][lay1]);
	}
}
...
void print_all_errors(int n, QUBIT **q_arr) {
	...
	for (i = 0; i < n; i++) {
		printf("q(%d)\n", i);
		qc_print_errors(q_arr[i]->error_cdll);
	}
}
...
int *infer_boundary(HT *dots, const int i1, const int j1, const int t1, const int t_offset) {
	...
	int *coord;
	std::cout << "infer_boundary" << std::endl;
...
void verify_balls_and_dots(QC *qc) {
	...
	std::cout << "\nprimal ball is associated to dot? ";
	n = qc->m_pr->dots->next;
	...
	// associated with a ball, and that the ball is associated with the dot. 
	std::cout << "\ndual ball is associated to dot? ";
	n = qc->m_du->dots->next;
